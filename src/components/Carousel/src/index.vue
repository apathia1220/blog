<template>
    <div v-if="data.length" class="relative m-1 z-10 shadow-lg opacity-100" @mouseenter="stopAutoPlay"
        @mouseleave="autoPlay">
        <!-- Carousel wrapper -->
        <div class="relative overflow-hidden rounded-xl h-ap-carousel">
            <div v-for="(item, index) in data" :key="index">
                <Transition class="absolute" name="carousel">
                    <div v-show="index === currentIndex"
                        class="grid grid-cols-2 grid-rows-none w-full h-ap-carousel absolute top-0 z-10 shadow-lg overflow-hidden bg-ap-card">
                        ...
                        <div class="flex flex-col relative p-12 z-40 row-auto text-ap-normal">
                            ...
                            <h1 class="text-title-1 mt-4 mb-8 font-extrabold cursor-pointer"
        ...
        <div class="absolute z-40 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
            <div v-for="(_, index) in data" :key="index" :style="{ opacity: index === currentIndex ? '1' : '' }"
                class="w-3 h-3 rounded-full cursor-pointer bg-ap-normal opacity-50 hover:opacity-100"
                @click="goTo(index)" />
        </div>
        <!-- Slider controls -->
        <div @click="goPrev"
    ...
    <div v-else class="relative h-ap-carousel m-1 z-10 shadow-lg">
        <!-- Carousel wrapper -->
        <div class="relative overflow-hidden rounded-xl">
            <div
                class="grid grid-cols-2 grid-rows-none w-full h-ap-carousel relative top-0 z-10 shadow-lg overflow-hidden bg-ap-card duration-700 ease-in-out">
                <div class="relative row-auto article-img">
                    <img class="block absolute z-20 left-0 bg-cover h-full w-12/10 max-w-12/10 object-cover"
