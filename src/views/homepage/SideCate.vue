<template>
    <div class="px-8 ml-6 my-8 rounded-2xl shadow-xl bg-ap-card">
        ...
        <div v-else>
            <div class="grid grid-cols-2 gap-2 pb-8" v-if="!!cateList && cateList.length !== 0">
                <p class="p-2 bg-ap-bg rounded-lg opacity-80 cursor-pointer
                            text-center text-xs text-ap-normal hover:text-ap-bright hover:opacity-100"
                    v-for="cate in cateList" :key="cate.id" @click="goArticle(cate.id, cate.categoryName)">
                    <span>{{ cate.categoryName }}</span>
                    <span class="ml-2 text-pink-500">{{ cate.articleCount }}</span>
                </p>
            </div>
...
import { getAllCates, type CateItem, type CateListResponse } from '@/apis/category'
...
const getTags = async () => {
    ...
    try {
        const res: CateListResponse = await getAllCates() as CateListResponse
...
const goArticle = (id: number, categoryName: string) => {
    router.push(`/article/?categoryId=${id}${categoryName ? `&categoryName=${categoryName}` : ''}`)
}
